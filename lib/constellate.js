...
module.exports = function(modName, serverAddr) {
    const ipfs = new Ipfs();
    let mod, proj;
    ...
    this.generate = (content, metadata, name, password) => {
        ...
        if (!password) {
            ...
            return Promise.all(promises).then(objs => {
                ...
                const keys = new File([JSON.stringify(keysObj, null, 2)], name + '_keys.json', {
...
function _ipldFromObjects(ipfs) {
    return (objs) => {
...
function getSubMatches(regex, str) {
    const subMatches = [];
    let match;
    while (true) {
