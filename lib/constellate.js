...
module.exports = function(repoPath = '/tmp/constellate', serverAddr = 'http://127.0.0.1:8888') {
    ...
    const fingerprint = (file, tasks, t, i) => {
    ...
    const getContent = (query, key, tasks, t, i) => {
    ...
    const importContent = (_files, password, tasks, t, i) => {
    ...
    this.generateIPLD = (cb) => {
        tasks.init(cb)
        generateIPLD(tasks, -1)
    }

    this.getContent = (query, cb) => {
    ...
    this.getMetadata = (query, expand, cb) => {
        tasks.init(cb)
