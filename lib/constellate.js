...
module.exports = function(repoPath = '/tmp/constellate', serverAddr = 'http://127.0.0.1:8888') {
                ...
                bcrypt.genSalt(10, (err, salt) => {
                    ...
                    bcrypt.hash(password, salt, (err, hash) => {
                        ...
                        try {
                            ...
                            files[i] = new File([data], files[i].name, {
                                type: files[i].type
                            })
    ...
    const getContent = (query, key, t, id) => {
        t.task(data => {
            if (key) {
                try {
    ...
    const importContent = (_files, password, t, id) => {
    ...
    const importMetadata = (files, t, id) => {
    ...
    this.getMetadata = (queries, expand, cb) => {
        t.callback(cb)
