...
const parseJSONs = (jsons, types, tasks, t, i) => {
...
const readFilesAs = (_files, readAs, tasks, t, i) => {
...
const hashPassword2x = (password, salt, tasks, t, i) => {
...
module.exports = function() {
    ...
    const importDecryption = (text, tasks, t, i) => {
        let dec = {}
        try {
            dec = JSON.parse(text)
        } catch (err) {
    ...
    const importProject = (project, tasks, t, i) => {
        if (!project.title) {
        ...
        for (let s = 0; s < project.sheets.length; s++) {
            ...
            objs = (Array).apply(null, {
                length: sheet.rows.length
            }).map(() => {
                return {
                    type: sheet.subject
                }
    ...
    const pushIPLD = (service, tasks, t, i) => {
        if (!ipld || !ipld.length) {
    ...
    const uploadContent = (service, tasks, t, i) => {
    ...
    */
    ...
    const encrypt = (password, tasks, t, i) => {
        if (!files || !files.length) {
            return tasks.error('no content')
        }
    ...
    */
    ...
    const generateIPLD = (service, tasks, t, i) => {
        if (!meta || !meta.length) {
    ...
    const getContent = (service, path, decrypt, tasks, t, i) => {
        let t1 = t,
            t2
    ...
    const importContent = (service, _files, password, tasks, t, i) => {
    ...
    const importMetadata = (files, tasks, t, i) => {
        meta = meta || []
    ...
    this.exportDecryption = () => {
        return decryption ? decryption : null
    }

    this.exportFileHashes = () => {
    ...
    this.importContent = (name, files, password, cb) => {
    ...
    this.importDecryption = (text, cb) => {
        tasks.init(cb)
        importDecryption(text, tasks, -1)
    }

    this.importKeypair = (keypair)

    this.importMetadata = (files, cb) => {
        tasks.init(cb)
        importMetadata(files, tasks, -1)
    }

    this.importProject = (project, cb) => {
        tasks.init(cb)
        importProject(project, tasks, -1)
    }

    this.generateIPLD = (name, cb) => {
        tasks.init(cb)
        tasks.add(service => {
    ...
    this.getContent = (service, path, decrypt, cb) => {
    ...
    this.getMetadata = (name, path, expand, cb) => {
        tasks.init(cb)
        let resolver
        tasks.add(service => {
            try {
    ...
    this.Browser.getContent = (service, path, decrypt, cb) => {
        if (typeof decrypt === 'function') {
            [cb, decrypt] = [decrypt, {}]
        } else if (!cb) {
            throw new Error('no callback')
    ...
    this.Browser.importContent = (name, files, password, cb) => {
        if (typeof password === 'function') {
            [cb, password] = [password, '']
        } else if (!cb) {
            throw new Error('no callback')
    ...
    this.Browser.importDecryption = (file, cb) => {
        tasks.init(cb)
        tasks.add(text => {
            importDecryption(text, tasks, -1)
        })
    ...
    this.Browser.importMetadata = (files, cb) => {
        tasks.init(cb)
        tasks.add(files => {
            importMetadata(files, tasks, -1)
        })
