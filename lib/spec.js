...
const mergeKeys = (obj, fn) => merge(...Object.keys(obj).map((k) => arrayToObject([[k, recurse(fn(obj[k], k), fn)]])));
...
const toElement = (obj, defs) => {
  if (!object(obj)) { return null; }
    switch(obj.type) {
      case 'array':
        const elem = toElement(obj.items, defs);
      ...
      default:
        ...
        if (object(def)) { return toElement(def, defs); }
    }
    return null;
}

const fromElement = (elem) => {
  switch(elem.nodeName) {
    case 'FIELDSET':
      const divs = Array.from(elem.children);
      if (!array(divs)) { return null; }
      const parsed = parseDIVs(divs);
      if (empty(parsed)) { return null; }
      return merge(parsed, {type: 'object'});
    case 'INPUT':
      switch(elem.type) {
        case 'checkbox':
          return {type: 'boolean'};
    ...
    case 'OL':
      const lis = Array.from(elem.children);
      if (!array(lis)) { return null; }
      return {
        'type': 'array',
        'items': fromElement(lis[0].children[0]),
        'minItems': 1,
        'uniqueItems': true
      }
    default:
...
function parseForm(form) {
  if (form.nodeName !== 'FORM') { return null; }
...
const parseDIVs = (divs) => {
  if (!array(divs)) { return null; }
  const required = (elem, label, reqs) => {
    if (reqs.includes(label.textContent) || !elem.hasAttribute('required')) { return reqs; }
    return toArray(reqs).concat(label.textContent);
  }
  return divs.reduce((result, div) => {
    if (empty(result) || div.nodeName !== 'DIV') { return null; }
    const children = Array.from(div.children);
    if (!array(children) || children.length !== 2) { return null; }
    const label = children[0];
    if (label.nodeName !== 'LABEL') { return null; }
    const elem = children[1];
    const obj = fromElement(elem);
    if (!object(obj)) { return null; }
    return merge(result, {
      properties: addValue(result.properties, label.textContent, attributes(elem, obj)),
      required: required(elem, label, result.required)
    });
  }, {'properties': {}, 'required': []});
}

const generateDIVs = (schema) => {
  const defs = recurseFreeze(schema.definitions);
  const props = recurseFreeze(schema.properties);
  const req = recurseFreeze(schema.required);
  const form = recurse(props, (obj, key) => {
    let elem = toElement(obj, defs);
    if (empty(elem)) { return null; }
...
const newComposition = (
  ...
  [
    ['arranger', _map(toArray(arranger), getHeader)],
    ['composer', _map(toArray(composer), getHeader)],
    ['iswc', iswc],
    ['lyricist', _map(toArray(lyricist), getHeader)],
    ['publisher', _map(toArray(publisher), getHeader)],
    ['recordedAs', _map(toArray(recordedAs), getHeader)],
    ['title', title]
  ], url
);
...
const newRecording = (
  ...
  [
    ['isrc', isrc],
    ['performer', _map(toArray(performer), getHeader)],
    ['producer', _map(toArray(producer), getHeader)],
    ['recordingOf', getHeader(recordingOf)],
    ['recordLabel', _map(toArray(recordLabel), getHeader)]
  ], url
);
