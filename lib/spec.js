...
const recurse = (obj, fn) => {
  if (!object(obj)) { return obj; }
  return merge(...Object.keys(obj).map((k) => arrayToObject([[k, recurse(fn(obj[k], k), fn)]])));
}

const recurseFreeze = (obj) => recurse(obj, Object.freeze);
...
const element = (obj, defs) => {
  if (!object(obj)) { return null; }
    switch(obj.type) {
      case 'array':
        const elem = element(obj.items, defs);
      ...
      case 'object':
        ...
        fieldset.appendChild(_form(obj));
        return fieldset;
      case 'string':
        return input('text');
      default:
        const def = definition(obj, defs);
        if (object(def)) { return element(def, defs); }
    }
    return null;
}
...
const _form = (schema) => {
  ...
  const form = recurse(props, (obj, key) => {
    let elem = element(obj, defs);
    if (empty(elem)) { return null; }
    elem = arrayFromObject(obj).reduce((result, [k, v]) => {
      return field(result, k, v);
    }, elem);
    if (array(req)) {
      if (req.some((k) => k === key)) {
        return field(elem, 'required', true);
      }
    }
    return elem;
  });
...
const setId = (obj) => merge(orderObject(obj), {id: base64_digest(withoutHeader(obj))});
const validateId = (obj) => obj.id === setId(obj).id;

const validate = (obj, schema) => {
  if (!validateId(obj)) { throw new Error(`Has invalid id: ${obj.id}`); }
  if (!validateSchema(obj, schema)) { throw new Error(`Has invalid schema: ${obj}`); }
  return null;
}
...
const newComposition = (
  composer, iswc, lyricist, publisher, recordedAs, title, url) => newModel(
  [
