...
const jti = Object.assign({}, IPFSHash, {
    readonly: true
});
...
function newEd25519Header(publicKey) {
    let header = {};
    try {
...
function newRsaHeader(publicKey) {
    let header = {};
    try {
...
function newSecp256k1Header(publicKey) {
    let header = {};
    try {
...
function signClaims(claims, header, privateKey) {
    let sig = Buffer.from([]);
...
function validateClaims(claims, meta, subject) {
    return new Promise((resolve, reject) => {
        ...
        if (claims.typ === 'Create') {
            if (validateMeta(meta)) {
                calcIPFSHash(meta).then((id) => {
                    if (claims.sub !== id) {
                        return reject(`expected sub=${claims.sub}; got ` + id);
                    }
                    switch (meta['@type']) {
                        case 'Album':
                        ...
                        case 'Recording':
                            if (!meta.performer.concat(meta.producer).some((id) => {
                                    return claims.iss === id;
                                })) return reject('iss should be performer or producer addr');
                            break;
                            //..
                    }
                    resolve(true);
...
function verifyClaims(claims, header, meta, signature, subject, subjectSignature) {
    return new Promise((resolve, reject) => {
