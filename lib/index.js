...
const _newAccount = (password, tasks, t, i) => {
...
MetadataService.prototype.put = function(password, cb) {
    if (typeof password === 'function') {
        [cb, password] = [password, '']
    }
    const tasks = new Tasks(cb)
...
MetadataService.prototype._decrypt = function(password, tasks, t, i) {
...
MetadataService.prototype._get = function(path, expand, id, tasks, t, i) {
    const hashes = this.hashes
    const parts = path.split('/')
    const first = parts.shift()
