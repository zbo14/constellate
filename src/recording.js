...
function Recording(
  isrc: ?string, performer: Header[], producer: Header[],
  recordingOf: Header, recordLabel: ?Header[], url: string) {
    let i;
    if (isrc != null) {
      this.isrc = isrc;
    }
...
Recording.prototype.addHeader = function(property: string, hasHeader: Header): boolean {
  switch (property) {
    case 'performer':
    case 'producer':
    case 'recordLabel':
...
Recording.prototype.validate = function(): ?Error {
