...
const orderObjects = (objs: Object[]): Object[] => {
...
module.exports = function(repoPath: string = '/tmp/constellate', serverAddr: string = 'http://127.0.0.1:8888') {
    ...
    const metaToIPLD = (t: Object, id?: number | string) => {
    ...
    this.fingerprint = (file: File, t: Object, id?: number | string) => {
        if (!serverAddr) {
    ...
    this.get = (query: string, key ? : string, t: Object, id?: number | string) => {
        t.prepend(obj => {
            t.next();
            t.run(obj, id);
        });
    ...
    this.importContent = (_files: File[], password: string, t: Object, id?: number | string) => {
    ...
    this.importMeta = (files: File[], t: Object, id?: number | string) => {
        if (!files || !files.length) {
            return t.error(new Error('no files'));
        }
    ...
    this.upload = (t: Object, id?: number | string) => {
        t.prepend(() => {
            t.next();
            t.run(id);
        });
